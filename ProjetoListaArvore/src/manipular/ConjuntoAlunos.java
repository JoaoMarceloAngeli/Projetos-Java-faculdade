 ...
 */
package manipular;
import Estrutura.Arvore_binaria;
import Estrutura.IArvore;
import java.util.ArrayList;
import java.util.List;
import modelo.Aluno;

public final class ConjuntoAlunos implements IAluno{
    ...
    private  List<Aluno> deletarArvores(String nomeMatricula, String tipoBusca) throws Exception{
        try {
            arvore = new Arvore_binaria();
            List<Aluno> dadosDelet = null;
                if(tipoBusca.compareTo("NOME") == 0){
                for(int i = 0;i < array.size(); i++){
                    arvore.incluir(array.get(i).getNome(), array.get(i));
                }
                ...
                dadosDelet = arvore.deletar(nomeMatricula);
                
                if(dadosDelet.isEmpty()) throw new Exception("(Nao Encontrado iremos fechar a tela\n)");
            
                return dadosDelet;
            
        } catch (Exception erro) {
        throw new Exception(erro.getMessage() + "\nerro no deletar aluno\n");
        } 
   }
    @Override
    public List<Aluno> deletar(String nomeMatricula, String tipoDelet) throws Exception {
        try {
            if( buscarArvores(nomeMatricula, tipoDelet).isEmpty()) throw new Exception("(Nao Encontrado iremos fechar a tela)\n");
            
            
            return deletarArvores(nomeMatricula, tipoDelet);
        } catch (Exception erro) {
        throw new Exception(erro.getMessage() + "erro no deletar aluno\n");
        }
    }
