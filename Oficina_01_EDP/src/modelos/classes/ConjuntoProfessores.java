...
public class ConjuntoProfessores implements IServicos{
    ...
    private int buscar(int matricula) {
        for (int i = 0; i <= indiceOcupacao; i++) {
            if (array[i].getMatricula() == matricula) return i;
        }
        return -1;
    }
...
public void excluir(int matricula) throws Exception {
    try{
    int indice = buscar(matricula);

    if (indice != -1) { 
        array[indice] = array[indiceOcupacao];
        array[indiceOcupacao] = null;
        indiceOcupacao--;
    }else{
        JOptionPane.showMessageDialog(null, "Matrícula não encontrada. Não foi possível excluir.");
    }
