...
private void abrirPdf() {
    ...
    private void jButton3_ExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3_ExcluirActionPerformed
        try {
            ...
            if (selecionado == -1) {
                JOptionPane.showMessageDialog(null, "Nenhuma linha selecionada. Selecione um contato para excluir.", "Erro", JOptionPane.ERROR_MESSAGE);
            return;
            }
    ...
    private void jTable1_TabelaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1_TabelaMouseClicked
        ...
        int selecionado = jTable1_Tabela.getSelectedRow();
        int modelRow = jTable1_Tabela.convertRowIndexToModel(selecionado);
        
        if (selecionado >= 0){
        String tblNome = table.getValueAt(modelRow, 1).toString();
        String tblTelefone = table.getValueAt(modelRow, 2).toString();
        String tblEmail = table.getValueAt(modelRow, 3).toString();
        String tblEndereco = table.getValueAt(modelRow, 4).toString();
    ...
    String telefone = jFormattedTextField1_Telefone.getText().trim().replaceAll("[^0-9]", "");
    String email = jTextField1_Email.getText();
    String logradouro = jTextField1_Logradouro.getText();
    String numeroStr = jTextField2_Numero.getText().trim().replaceAll("[^0-9]", "");
    ...
    try {
        ...
        if (telefone.isEmpty()) erro += "Telefone não pode estar vazio.\n";
        if (email.isEmpty()) erro += "Email não pode estar vazio.\n";
        if (logradouro.isEmpty()) erro += "Logradouro não pode estar vazio.\n";
        if (numeroStr.isEmpty()) erro += "Número não pode estar vazio.\n";
        if (cep.isEmpty()) erro += "Cep não pode estar vazio.\n";
        if (cidade.isEmpty()) erro += "Cidade não pode estar vazia.\n";
        if (estado.isEmpty()) erro += "UF não pode estar vazia.\n";

        if (!erro.isEmpty()) {
            throw new Exception(erro);
        }

        int numero = Integer.parseInt(numeroStr); // Só converter após verificar que não está vazio
        ...
        while (!confirmado) {
            ...
            String numeroCompleto = "+" + contato.getTelefone().getDdi() + " " +
            ...
            if (resposta == JOptionPane.YES_OPTION) {
                ...
                contato.getTelefone().setDdi(Integer.parseInt(novoDdi));
                contato.getTelefone().setDdd(Integer.parseInt(novoDdd));
                contato.getTelefone().setNumero(Integer.parseInt(novoNumero));
            } else {
